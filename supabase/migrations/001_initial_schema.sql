...
CREATE TABLE IF NOT EXISTS stations (
  ...
  line         TEXT NOT NULL CHECK (line IN ('red', 'green', 'purple')),
  lat          DOUBLE PRECISION NOT NULL,
  lon          DOUBLE PRECISION NOT NULL,
  geom         GEOMETRY(Point, 4326),                 -- PostGIS point
  station_type TEXT CHECK (station_type IN (
                 'commuter', 'transfer', 'residential',
                 'mixed', 'central', 'tourist', 'business'
               )),
  opened_year  INTEGER,
...
SELECT
  ...
  CASE
    WHEN f.baseline_entries > 0 THEN
      ROUND(((f.predicted_entries - f.baseline_entries)::REAL / f.baseline_entries) * 100, 1)
    ELSE 0
  END AS delta_pct
FROM stations s
